...
data = readtable('single_5_cm.csv');  % Replace with your CSV filename
V = data.V;           % Voltage (V)
...
VT = 25.85e-3;                % Thermal voltage (approx) at room temp (V)
Isc = max(I);                 % Short-circuit current (A)
dVdI = 1 / dIdV(idxVoc);      % Reciprocal of dI/dV
Rs = -dVdI - (VT / Isc);      % From the thesis approximation
